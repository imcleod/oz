...
class RedHatCDYumGuest(RedHatCDGuest):
    ...
    def _internal_customize(self, libvirt_xml, action):
        ...
        icicle = None
        try:
            libvirt_snapshot = None
            if action == "gen_only":
                self.log.debug('Using "safe" snapshot based ICICLE generation for gen_only requiest')
                (libvirt_dom, libvirt_snapshot, modified_disk) = self._create_snapshot(modified_xml)
            else:
            ...
            if libvirt_snapshot:
                # Note that there is no need to _collect_teardown as we simply delete the disk containing those changes
                self._restore_snapshot(libvirt_dom, libvirt_snapshot, modified_disk)
            else:
                self._collect_teardown(modified_xml)

        return icicle

    def _restore_snapshot(self, libvirt_dom, libvirt_snapshot, modified_disk):
        libvirt_snapshot.delete(libvirt.VIR_DOMAIN_SNAPSHOT_DELETE_METADATA_ONLY)
        libvirt_dom.undefine()
        os.unlink(modified_disk)

    def _create_snapshot(self, modified_xml):
        # Since we are not modifying here we can use a snapshot to avoid any unwanted side effects from booting the guest.
        # The mechanics are slightly confusing.  When creating the snapshot a new disk files is generated by libvirt.
        # This new disk file is a qcow2 writeable snapshot regardless of whether the original disk is raw or qcow2.
        ...
        # Now that we have taken a snapshot, libvirt_dom's disk is backed by a writeable qcow2 snapshot.
        # We want to discard this when we are done but libvirt does not yet have a way to delete the file for us.
        # So, we determine its name here and unlink it later.
        doc = libxml2.parseDoc(libvirt_dom.XMLDesc(0))
        disksources = doc.xpathEval("/domain/devices/disk/source")
        if len(disksources) != 1:
            self.log.warning("Oz given a libvirt domain with more than 1 disk; using the first one parsed")
        modified_disk = disksources[0].properties.get_content()

        # Now that we have taken a snapshot we can also do the collect_setup mods without changing our original disk
        # We pass in the current XML description of the domain, which points to our writeable snapshot disk
        self._collect_setup(libvirt_dom.XMLDesc(0))
        
        # Now we can actually launch the domain
        libvirt_dom.create()

        return (libvirt_dom, libvirt_snapshot, modified_disk)
